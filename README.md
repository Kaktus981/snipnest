...
> 当前版本为 `v0.2.6`，通过 GitHub Release 手动安装。首次使用建议先在不重要的网页内容上熟悉恢复、删除和更新流程。
...
如果Edge在更新后提示新增的标签页或浏览记录权限，请确认后重新启用扩展。该权限只用于让直接打开的侧边栏识别当前活动标签页的网址和标题；网页正文和输入内容仍需逐站授权。
...
- 为支持从Edge侧边栏直接识别当前网站，扩展可以读取当前活动标签页的网址和标题；这不包含网页正文或表单输入。
...
| `alarms` | 每日清理已过期的临时草稿 |
| `tabs` | 识别当前活动标签页的网址和标题，使直接打开的侧边栏也能显示并授权当前网站 |
| 可选网站权限 | 仅在用户确认后访问对应的 `http` 或 `https` 网站来源 |

`tabs` 不提供网页正文或输入框内容。文栈不会在安装时默认获得所有网站内容权限；只有用户逐站确认后才注册内容脚本。撤销授权后，扩展会停止监听并注销该网站的内容脚本。
...
本项目采用 [MIT License](LICENSE)。你可以自由使用、复制、修改、合并、发布、分发、再授权和销售本项目副本，但必须保留版权声明和许可证文本。软件按“现状”提供，具体免责声明见许可证文件。

界面内置字体分别采用 SIL Open Font License 1.1，许可原文见 [第三方许可证目录](docs/third-party-licenses)。正式安装包也包含相同许可证文本。
