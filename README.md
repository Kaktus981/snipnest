...
> 当前版本为 `v0.2.2`，通过 GitHub Release 以开发者模式安装。首次使用建议先在测试网页或不重要的内容上熟悉恢复、删除和更新流程。

[下载最新版本](https://github.com/Kaktus981/snipnest/releases/latest) · [安装教程](#下载与安装) · [完整测试教程](测试教程.md) · [功能规划](docs/feature-catalog.md) · [反馈问题](https://github.com/Kaktus981/snipnest/issues)
...
- **本地数据控制**：支持 JSON 导出和一键清除全部数据，不需要账号、云端服务或遥测。
...
- “清除全部数据”会删除草稿、片段、设置和授权记录，操作不可撤销。

导出的 JSON 包含文稿正文，请保存到可信位置，不要上传到公开 Issue 或公共网盘。
...
- 安装包和源码不包含开发者本人的草稿；每位用户的数据只存在于自己的 Edge 配置中。
...
npm run build      # 生成图标、检查 TypeScript 并构建 dist
npm run test-site  # 在 127.0.0.1:4173 启动本地测试站
```

生产构建位于 `dist/`，可在 Edge 的 `edge://extensions` 中使用“加载解压缩的扩展”直接加载。

## 完整功能测试

Windows 用户可以双击项目根目录的 `启动测试网站.cmd`，然后在 Edge 打开：

```text
http://127.0.0.1:4173/test-site/
```

测试站完全在本机运行，不会上传测试文字。请按照 [测试教程.md](测试教程.md) 依次验证安装与授权、1 字保存、短草稿折叠、历史版本、误删恢复、三种分组、批量操作、敏感字段过滤、片段推荐和权限撤销。

## 项目结构

| 目录或文件 | 说明 |
| --- | --- |
| `src/background.ts` | 授权、动态脚本注册、草稿清理和统一消息处理 |
| `src/content.ts` | 字段发现、输入采集、恢复和片段插入 |
| `src/popup/` | 当前网站授权、暂停保护和打开侧边栏 |
| `src/sidepanel/` | 当前页面、草稿、片段及隐私设置界面 |
| `src/shared/` | 数据库、类型、字段逻辑、分组逻辑及测试 |
| `test-site/` | 覆盖标准表单、动态字段、富文本和敏感字段的本地测试站 |
| `docs/feature-catalog.md` | 已启用功能和后续候选功能库 |
